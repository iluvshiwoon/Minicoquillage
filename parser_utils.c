...
{
	atom->std_in = wrap_malloc(mini, sizeof(*atom->std_in) * (count.std_in
				+ 1));
	atom->std_in[count.std_in] = NULL;
	atom->std_out = wrap_malloc(mini, sizeof(*atom->std_out) * (count.std_out
				+ 1));
	atom->std_out[count.std_out] = NULL;
	atom->std_order = wrap_malloc(mini, sizeof(char) * (count.std_out
				+ count.std_in + 1));
	atom->std_order[count.std_in + count.std_out] = '\0';
	atom->append = wrap_malloc(mini, sizeof(*atom->append) * (count.std_out));
	atom->args = wrap_malloc(mini, sizeof(*atom->args) * (count.args + count.cmd
				+ 1));
	atom->args[count.args + count.cmd] = NULL;
	atom->heredoc_eof = wrap_malloc(mini, sizeof(*atom->heredoc_eof)
			* (count.heredoc_eof + 1));
