...
/*   Updated: 2024/12/03 15:18:24 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	r_echo(int opt)
{
	...
	if (!opt)
		r_value = write(1, "\n", 1);
	if (r_value != 1 && !opt)
		return (ft_printf_fd(STDERR_FILENO, "minicoquillage: e\
cho: write error: No space left on device\n"), 1);
	return (0);
}
...
int	mini_echo(char **args)
{
	...
	while (args[index.i])
	{
		...
		if (!first)
		{
			r_value = write(1, " ", 1);
			if (r_value != 1)
				return (ft_printf_fd(STDERR_FILENO, "mi\
nicoquillage: echo: write error: No space left on device\n"), 1);
		}
		r_value = write(1, args[index.i] + index.j, ft_strlen(args[index.i])
				- index.j);
		if (r_value != (long long)ft_strlen(args[index.i]) - index.j)
			return (ft_printf_fd(STDERR_FILENO, "minicoquillag\
e: echo: write error: No space left on device\n"), 1);
		index.i++;
		first = false;
	}
	return (r_echo(opt));
