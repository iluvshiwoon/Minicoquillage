...
/*   Updated: 2024/12/02 03:16:25 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (path[++i])
	{
		...
		if (access(r_value, F_OK) == 0)
		{
			if (is_dir(r_value))
				...
				return (r_value);
			return (mini->status = 126, ft_printf_fd(STDERR_FILENO, "mini\
coquillage: %s: Permission denied\n", r_value), NULL);
		}
	}
	return (mini->status = 127, ft_printf_fd(STDERR_FILENO, "minicoquillage: %s",
			mini_ft_strjoin(mini, cmd, ": command not found\n")), NULL);
}
...
char	*_get_path(t_mini *mini, char *cmd)
{
	...
	if (!r_value) return (mini->status = 127, ft_printf_fd(STDERR_FILENO, "%s",
	...
	while (path[++i])
	{
		...
		if (r_value && r_value != path[i] && !*(r_value + ft_strlen(cmd)))
		{
			if (access(path[i], X_OK) == 0)
				return (path[i]);
			return (mini->status = 126, ft_printf_fd(STDERR_FILENO, "minic\
...
char	*get_path(t_mini *mini, char *cmd)
{
	...
	if (is_path(cmd) && access(cmd, F_OK) == -1)
		...
		return (mini->status = 126, ft_printf_fd(STDERR_FILENO, "minicoquillage:\
 %s: Permission denied\n", cmd), NULL);
