...
/*   Updated: 2024/11/27 18:27:43 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	_count_exp(t_mini *mini, char *str, int *i, t_to_expand _expand)
{
	...
	{
		var = wrap_malloc(&mini->heap_allocated, mini->heap.list, sizeof(*var)
				* (k - *i + 1));
...
{
	(*r_value) = wrap_malloc(&mini->heap_allocated, mini->heap.list,
			sizeof(char) * 2);
	(*r_value)[0] = '$';
	(*r_value)[1] = '\0';
}
