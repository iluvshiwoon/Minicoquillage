...
char * __get_path(t_heap * heap,char ** path,int * status, char * cmd)
{
    ...
    while (path[++i])
    {
        ...
        if (access(r_value, F_OK) == 0)
        {
            ...
            return(*status = 126,ft_printf_fd(STDERR_FILENO,\
                                              "minicoquillage: %s: Permission denied\n",r_value),NULL);
        }
    }
    return(*status = 127,ft_printf_fd(STDERR_FILENO,"minicoquillage: %s",\
                                      mini_ft_strjoin(heap->heap_allocated,heap->list,cmd,": command not found\n")),NULL);
...
char * _get_path(t_heap * heap,char ** envp,int * status, char * cmd)
{
    ...
    if (!r_value)
        return (*status = 127,ft_printf_fd(STDERR_FILENO,"%s",mini_ft_strjoin\
(heap->heap_allocated,heap->list,mini_ft_strjoin(heap->heap_allocated,heap->list\
,"minicoquillage: ",cmd),": No such file or directory\n")),NULL);
