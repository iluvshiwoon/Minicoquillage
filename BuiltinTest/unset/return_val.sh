...
while IFS=' ' read -r unset_val; do
	unset $unset_val
	val_return=$?
	echo $val_return >> val_return.txt
...
while IFS=' ' read -r unset_val; do
	./ft_unset $unset_val 2>>ft_unset_status.txt >tmp.txt
	output=$?
	echo $output >> ft_val_return.txt
	# Rechercher la variable dans le fichier
	if grep -q "^$unset_val=" tmp.txt; then
...
diff val_return.txt ft_val_return.txt
rm val_return.txt
rm ft_val_return.txt
rm tmp.txt
