...
/*   Updated: 2024/10/05 15:10:01 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int fill_atom(t_ast_node ** current_node, t_double_link_node * beg, t_double_link_node * end)
{
    t_parser_node * p_node;
    t_token_count count;
                 ...
                 t_double_link_node * end, t_ast_node ** current_node)
{
    ...
    *(*current_node)->left = (t_ast_node){};
    ...
    fill_atom(current_node,beg,end);
...
void skip_through(t_parser_node * p_node, t_double_link_node * next_op)
{
    t_token * token;
    t_token_count count;
...
void skip_through_parenthesis(t_parser_node * p_node, t_double_link_node * beg, t_double_link_node * next_op)
{
    ...
    if (token->type == OPEN_PARENTHESIS)
    {
        ...
        if (i && i->next && i->next != next_op)
        {
            ...
            *p_node->atom = (t_atom){};
            ...
            alloc_atom(t_count, p_node->atom);
