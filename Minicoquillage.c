...
/*   Updated: 2024/11/27 20:24:26 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    char ** new_env;
    new_env = wrap_malloc(&mini->heap_allocated, mini->heap_allocated.env, sizeof(char *)*4);
    new_env[0] = mini_ft_strdup(&mini->heap_allocated, mini->heap_allocated.env,"SHLVL=1");
    new_env[1] = mini_ft_strdup(&mini->heap_allocated, mini->heap_allocated.env,"PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin");
    new_env[2] = mini_ft_strdup(&mini->heap_allocated, mini->heap_allocated.env,"SHELL=env");
    new_env[3] = NULL;
    return (new_env);
}
