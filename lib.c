...
/*   Updated: 2024/12/03 15:16:27 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (pipe((*pipefd)[i]) == -1)
		return (close_fds(mini->fds), free_heap(mini, true),
			ft_printf_fd(STDERR_FILENO, "minicoquillage: pipe: %s\n",
...
int	m_open(t_mini *mini, const char *pathname, int flags, mode_t mode)
{
	...
	if (*fd == -1 && errno != ENOENT)
		return (close_fds(mini->fds), free_heap(mini, true),
			ft_printf_fd(STDERR_FILENO, "minicoquillage: open: %s\n",
...
void	m_dup2(t_mini *mini, int fd1, int std)
{
	...
	if (r == -1)
		return (close_fds(mini->fds), free_heap(mini, true),
			ft_printf_fd(STDERR_FILENO, "minicoquillage: dup2: %s\n",
...
int	m_dup(t_mini *mini, int std)
{
	...
	*fd = dup(std);
	if (*fd == -1)
		return (close_fds(mini->fds), free_heap(mini, true),
			ft_printf_fd(STDERR_FILENO, "minicoquillage: dup: %s\n",
